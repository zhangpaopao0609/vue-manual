...
有时候我们会设置一些常量，比如 `SOME_CONSTANT`，它其实一直也不会发生变化，但因为 `{{ }}` 的缘故，它会被打上 PatchFlag.TEXT 的标记，渲染器会将其放到 dynamicChildren 中，每次重渲染时，它都会进行一次比较，其实是没有这个必要的，因为我们知道它是不会发生变化的。那么我们可以在元素上添加 `v-once` 的标签：
