...
// 第二个 effect 开始执行
// p.push(1); 什么都不会发生
// console.log('1->', p, p.length); 桶里面收集 length => fn1， 此时 activeEffectFn = fn1
// 第一个 effect 执行完成，此时，activeEffectFn = null

// 第二个 effect 开始执行，activeEffectFn = fn2
// p.push(2) -> 设置 length，触发 length 对应的副作用函数，因为 activeEffectFn = fn2，所以 fn1 会成功从副作用函数 set 中拿出执行
  // fn1 开始执行，activeEffectFn = fn1
    // p.push(1) -> 设置 length，触发 length 对应的副作用函数，此时的 activeEffectFn = fn1，所以 fn1 不会从副作用函数 set 中拿出，
    // 继续执行 `console.log('1->', p, p.length);` 打印结果
  // fn1 执行完成
// 第二个 effect 执行完成
